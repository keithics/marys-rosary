...
struct RosaryView: View {
    ...
    var body: some View {
        GeometryReader { geo in
            ...
            let cam = camera(in: geo.size, points: points, pad: crystalD * 0.7)
            ...
            ZStack(alignment: .topLeading) {
                ...
                ForEach(Array(rosary.sequence.enumerated()), id: \.offset) { index, bead in
                    ...
                    let visibleD: CGFloat = isMedal ? medalD
                        : isCrucifix ? crucifixH * 0.604 * 0.5
                        : (bead.kind == .gold ? goldD * 0.85 : crystalD * 0.85)
    ...
    private func camera(in size: CGSize, points: [CGPoint], pad: CGFloat) -> Camera {
        switch mode {
        case .focused:
            let s: CGFloat = 1.45
