...
struct MysteryDelayRow: View {
    ...
    private var delay: Int { delayStore.responseSeconds(for: prayer) }
    ...
    var body: some View {
        VStack(spacing: 0) {
            HStack(spacing: 12) {
                ...
                HStack(spacing: 0) {
                    Button { delayStore.setResponseSeconds(delay - 1, for: prayer) } label: {
                        Image(systemName: "minus")
                            .font(.system(size: 13, weight: .semibold))
                            .frame(width: 36, height: 36)
                            .foregroundStyle(delay > 1 ? theme.accent : theme.textSubtle.opacity(0.3))
                    }
                    .buttonStyle(.plain)
                    .disabled(delay <= 1)
                    ...
                    Button { delayStore.setResponseSeconds(delay + 1, for: prayer) } label: {
                        Image(systemName: "plus")
                            .font(.system(size: 13, weight: .semibold))
                            .frame(width: 36, height: 36)
                            .foregroundStyle(delay < 90 ? theme.accent : theme.textSubtle.opacity(0.3))
                    }
                    .buttonStyle(.plain)
                    .disabled(delay >= 90)
                }
